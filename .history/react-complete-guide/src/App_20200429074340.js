...
class App extends Component {
  ...
  nameChangedHandler = (event, id) => {
    ...
    console.log("TCL: App -> nameChangedHandler -> person", personIndex)
      ...
      persons = (
        <div>
          {this.state.persons.map((person, index) => {
            return <Person {...person} key={perosn.id} changed={(event) => this.nameChangedHandler(event, person.id)} click={() => this.deletePersonHandler(index)} />
          })}
        </div >
      )
    }
